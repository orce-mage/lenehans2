...
var textEditHelper = (function($){
    ...
    var initEditEvent = function() {
        ...
        $('input[data-action="link-color"]').unbind( "change" ).change(function() {
            if (isActiveLink()) {
                doLinkColor($(this).val());
            }
        });
    };
    ...
    var initLinkColor = function (element) {
        var rgbColor = element.css('color');
    ...
    var doLinkColor = function (color) {
        var currentElm = $('.'+config.classes.helperLink);
        currentElm.css('color', color);
    };
