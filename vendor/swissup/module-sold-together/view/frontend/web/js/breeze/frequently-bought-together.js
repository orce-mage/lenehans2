/* global breeze soldTogetherHelper $t */
(function () {
    'use strict';
    ...
    breeze.widget('frequentlyBoughtTogether', {
        ...
        updateTotals: function () {
            ...
            if (this.options.taxDisplay === '3') {
                $(elIncTax).html($.catalog.priceUtils.formatPrice(totalPrice, this.options.priceFormat));
                $(elExclTax).html($.catalog.priceUtils.formatPrice(totalExclPrice, this.options.priceFormat));
            } else {
                $(elTotal).html($.catalog.priceUtils.formatPrice(totalPrice, this.options.priceFormat));
            }
        },
        ...
        addToCartItems: function () {
            ...
            this.getItems().each(function () {
                var $checkbox = $('.checkbox', this),
                    $productOptions = soldTogetherHelper.findProductOptions(this),
                    itemSuper = {};
            ...
            soldTogetherHelper.setRelatedIds(submitIds);
            soldTogetherHelper.setRelatedSupers(submitSuperAttribute);

            $('#product-addtocart-button').get(0).click();
        },
        ...
        validate: function () {
            var $messages,
                isValid = true,
                validator = window.soldTogetherValidator;
            ...
            window.scrollTo(0, $messages.offset().top - 100);
        ...
        restoreRelatedProductsField: function () {
            if (this.valuesToRestore !== false) {
                soldTogetherHelper.setRelatedIds(this.valuesToRestore.ids);
                soldTogetherHelper.setRelatedSupers(this.valuesToRestore.superAttribute);
                this.valuesToRestore = false;
            }
        },
