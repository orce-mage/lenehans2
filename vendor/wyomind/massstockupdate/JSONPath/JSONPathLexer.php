...
class JSONPathLexer
{
    ...
    // Eg. foo or 40f35757-2563-4790-b0b1-caa904be455f
    ...
    protected function createToken(string $value) : JSONPathToken
    {
        ...
        if (preg_match('/^(' . static::MATCH_INDEX . ')$/xu', $tokenValue, $matches)) {
            ...
            $tokenValue = ['start' => isset($parts[0]) && $parts[0] !== '' ? (int) $parts[0] : null, 'end' => isset($parts[1]) && $parts[1] !== '' ? (int) $parts[1] : null, 'step' => isset($parts[2]) && $parts[2] !== '' ? (int) $parts[2] : null];
        ...
        } elseif (preg_match('/^' . static::MATCH_INDEX_IN_SINGLE_QUOTES . '$/xu', $tokenValue, $matches) || preg_match('/^' . static::MATCH_INDEX_IN_DOUBLE_QUOTES . '$/xu', $tokenValue, $matches)) {
            if (isset($matches[1])) {
